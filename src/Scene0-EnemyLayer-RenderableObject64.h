...
class Scene0;
class EnemyLayer;
class RenderableObject64 : public ::gc::TypeAliases<RenderableObject64>{
	friend class ::gc::Camera;
	friend class EnemyLayer;
	typedef RenderableObject64 this_t;
	RenderableObject64 & self;
	Scene0 & scene;
	EnemyLayer & layer;
	public:
	...
	bool isVisible = true;
	bool isActive = true;
	RenderableObject64(Scene0 &, EnemyLayer &);
	~RenderableObject64();
	void onStart();
	...
	lref_t	 moveOn(const gc::Vec2 & v)noexcept {pos += v; return *this;}
		lref_t	 moveTo(const gc::Vec2 & v)noexcept {pos = v;  return *this;}
			private:
			RenderableObject64(const RenderableObject64 &) = delete;
			RenderableObject64(RenderableObject64 &&) = delete;
			void operator = (const RenderableObject64 &) = delete;
			void operator = (RenderableObject64 &&) = delete;
		};
