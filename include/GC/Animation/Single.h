...
{
	template<>
	class Animation<AnimationType::Single> : public ClassTraits<Animation<AnimationType::Single>>
	{
	...
	/*-----------------------------------IMPLEMENTATION----------------------------------------------*/
#pragma region GC_ANIMATION_SINGLE_REGION
	...
	const bool 		GC_ANIMATION_SINGLE::isPlay() const noexcept{
		return _isPlays;
	}
	...
	const size_t 	GC_ANIMATION_SINGLE::getCurrentFrameIndex() const noexcept{
		return _currentAnimationFrameIndex;
	}
	GC_ANIMATION_SINGLE::lref_t GC_ANIMATION_SINGLE::setCurrentSpeed(float s) noexcept{
		_currentAnimationSpeed = s;
		return *this;
	}
	GC_ANIMATION_SINGLE::lref_t GC_ANIMATION_SINGLE::start() noexcept{
		_isPlays = true;
		_onStart.emit(*this);
		return *this;
	}
	GC_ANIMATION_SINGLE::lref_t GC_ANIMATION_SINGLE::pause() noexcept{
		_isPlays = false;
		_onPause.emit(*this);
		return *this;
	}
	GC_ANIMATION_SINGLE::lref_t GC_ANIMATION_SINGLE::stop() noexcept{
		_isPlays = false;
