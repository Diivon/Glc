...
namespace gc
{
	...
	public:
		...
		const AnimationDirection getCurrentDirection() const noexcept;
		const AnimationType getCurrentType() const noexcept;

		//setters
		lref_t setCurrentSpeed(float s) noexcept;
		lref_t setCurrentDirection(AnimationDirection) noexcept;
		lref_t setCurrentType(AnimationType) noexcept;
...
#pragma region GC_ANIMATION_DYNAMIC_REGION
	...
	const uint32 	GC_ANIMATION_DYNAMIC::getCurrentFrameIndex() const noexcept{
